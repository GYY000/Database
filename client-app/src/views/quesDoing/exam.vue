<template>
  ...
  <div v-if="judge_mode === false">
    <div class="lower_panel" id="lower_panel">
      <el-row style="width: 100%" class="vertical-align-center">
        <el-col v-if="exam_mode" :span="3" style="height: 100%; margin-left: 30px;display: flex;align-items: center"
        ...
        <el-col v-else :span="3" style="height: 100%; margin-left: 30px;display: flex;align-items: center"
                @click="go_back" class="hover_class">
          <el-icon style="width: 40px; height: 45px;margin-top: 12px">
            <ArrowLeft/>
          </el-icon>
        ...
        <el-col :span="5">
          <el-button v-if="exam_mode" type="success" style="min-width: 80px;margin-right: 10px" plain round
                     @click="open_hand_in">
            提交
          </el-button>
          <el-button text :icon="Timer">
            <span style="font-size: 16px">{{ diff_time }}</span>
          </el-button>
        </el-col>
      </el-row>
    </div>
    ...
    <div class="center_class">
      <div class="main_container" id="main_container">
        ...
        <div v-if="exam_mode" class="row_margin">
        ...
        <div v-if="exam_mode === false" style="display: flex;justify-content: center;
        font-size: 30px;color: grey;font-family: 'Microsoft YaHei'">
          距离考试开始还有
        </div>
        <div v-if="exam_mode === false" style="display: flex;
        justify-content: center;font-size: 25px;color: #3cbcd0;font-weight: bold;
            font-family: 'Microsoft YaHei'">
          {{ diff_time }}
        </div>
        <div v-if="exam_mode === false" style="display: flex;justify-content: center">
          <img src="@/assets/image/沙漏.gif" style="width: 684px;height: 384px;border-radius: 6%"/>
        </div>
        <div v-if="show && exam_mode === true" style="margin-top: 15px">
          <el-pagination
  ...
  <div class="right-panel1" v-if="panel_switch && judge_mode === false && exam_mode === true">
  ...
  <div class="right-panel2" v-if="panel_switch === false && judge_mode === false && exam_mode === true">
...
import {fetch_exam_info, fetch_ques_info, hand_in_ans, string2Array} from "@/views/main/api";
...
export default {
  ...
  setup() {
    ...
    const init = () => {
      ...
      fetch_exam_info(router.currentRoute.value.params.eid).then(
          (res) => {
            qsid.value = res.qsid
